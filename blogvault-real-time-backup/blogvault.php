...
if (BVHelper::getRawParam('REQUEST', 'bvplugname') == "bvbackup") {
	...
	if ($bvinfo->hasValidDBVersion()) {
		if ($bvinfo->isProtectModuleEnabled()) {
			...
			BVProtect_V644::$settings = new BVWPSettings();
			BVProtect_V644::$db = new BVWPDb();
			BVProtect_V644::$info = new BVInfo(BVProtect_V644::$settings);

			add_action('bv_clear_pt_config', array('BVProtect_V644', 'uninstall'));

			if ($bvinfo->isActivePlugin()) {
				BVProtect_V644::init(BVProtect_V644::MODE_WP);
			}
		}
