...
if (!trait_exists('BVProtectFWRuleArrayFunc_V662')) :
trait BVProtectFWRuleArrayFunc_V662 {
	...
	private function validateKeyMatchPattern($pattern) {
		...
		try {
			$resp = BVHelper::safePregMatch($pattern, '');
		} finally {
	...
	private function keyMatchesPattern($pattern, $key) {
		$resp = BVHelper::safePregMatch($pattern, (string) $key);
		if ($resp === false) {
			throw new BVProtectRuleError_V662(
	...
	private function keyMatchesRecursive($array, $pattern, &$traversed_keys, $depth = 1) {
		if ($depth > BVProtectFWRuleEngine_V662::MAX_DEPTH_TO_ALLOWED_TYPE_FUNC) {
			return false;
		}
	...
	private function enforceArrayTraversalLimit($func_name, $traversed_keys) {
		if ($traversed_keys > BVProtectFWRuleEngine_V662::MAX_ARRAY_KEYS_TO_TRAVERSE) {
			throw new BVProtectRuleError_V662(
				$this->addExState(
					"TraversalLimitError: " . $func_name . " exceeded " .
					BVProtectFWRuleEngine_V662::MAX_ARRAY_KEYS_TO_TRAVERSE .
					" array keys"
				)
			);
		}
