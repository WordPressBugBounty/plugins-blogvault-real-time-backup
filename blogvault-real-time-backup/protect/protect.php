...
if (!class_exists('BVProtect_V648')) :
...
class BVProtect_V648 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V648::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V648::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V648::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V648($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V648::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V648::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V648::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V648::uninstall();
		BVProtectFW_V648::uninstall();
		BVProtectLP_V648::uninstall();

		BVProtect_V648::removeWPPrepend();
		BVProtect_V648::removePHPPrepend();
		BVProtect_V648::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V648::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V648::removeHtaccessPrepend();
		BVProtect_V648::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V648::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V648::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V648::rrmdir($mc_data_dir);
	}
}
endif;