...
if (!class_exists('BVProtect_V644')) :
...
class BVProtect_V644 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V644::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V644::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V644::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V644($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V644::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V644::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V644::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V644::uninstall();
		BVProtectFW_V644::uninstall();
		BVProtectLP_V644::uninstall();

		BVProtect_V644::removeWPPrepend();
		BVProtect_V644::removePHPPrepend();
		BVProtect_V644::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V644::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V644::removeHtaccessPrepend();
		BVProtect_V644::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V644::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V644::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V644::rrmdir($mc_data_dir);
	}
}
endif;