...
if (!class_exists('BVProtect_V662')) :
...
class BVProtect_V662 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V662::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V662::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V662::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V662($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V662::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V662::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V662::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V662::uninstall();
		BVProtectFW_V662::uninstall();
		BVProtectLP_V662::uninstall();

		BVProtect_V662::removeWPPrepend();
		BVProtect_V662::removePHPPrepend();
		BVProtect_V662::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V662::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V662::removeHtaccessPrepend();
		BVProtect_V662::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V662::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V662::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V662::rrmdir($mc_data_dir);
	}
}
endif;